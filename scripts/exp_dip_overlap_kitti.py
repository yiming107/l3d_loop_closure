...
# Experiment setup: this experiment follows the OverlapNet loop closure detection setup
# where the camera poses and the covariance are exploited to select the candidate frame.
# The 'pair_indices.json' includes all the candidate pairs obtained using the script provided in OverlapNet repo.
...
if __name__ == '__main__':
    ...
    pair_gap = 4  # compute the transform for every 4 candidate frames to reduce the computational load
    pairs_json_file = os.path.join(result_folder, 'pair_indices.json')
    result_json_file = os.path.join(result_folder, 'dip_overlap_result.json')
